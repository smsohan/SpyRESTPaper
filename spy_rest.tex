...
\textbf{The Spy:} SpyREST is driven by example API calls. The Spy component is a HTTP proxy server that needs to be used while running example API calls so that it can record HTTP request and response information in a database that are required to auto generate the API documentation. For example, when using Spy as a HTTP proxy to make the following HTTP request:
